...
namespace Bluejam.Utils.DatabaseScripter.Core.Manifest
{
    ...
    public class Manifest
    {
        ...
        {
            var script = UnversionedScripts.Find(item => string.Equals(item.Name, name, StringComparison.OrdinalIgnoreCase));
            if (script == null)
            {
                script = VersionedScripts.Find(item => string.Equals(item.Name, name, StringComparison.OrdinalIgnoreCase));
            }
            return script;
        }
