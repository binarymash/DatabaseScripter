...
namespace Palmmedia.ReportGenerator.Parser
{
    ...
    public class NCoverParser : ParserBase, IParser
	{
            ...
            : base(report)
        {
            ...
            foreach (var assembly in this.Assemblies())
            {
                foreach (var clazz in this.ClassesInAssembly(assembly))
                {
                    ...
                    {
                        var seqpntsOfFile = report.Descendants("module").Where(type => type.Attribute("assembly").Value.Equals(assembly)).Elements("method").Where(method => method.Attribute("class").Value.StartsWith(clazz, StringComparison.Ordinal)).Descendants("seqpnt").Where(seqpnt => seqpnt.Attribute("document").Value.Equals(file)).OrderBy(seqpnt => int.Parse(seqpnt.Attribute("line").Value, CultureInfo.InvariantCulture));
        ...
        {
            return this.Report.Descendants("seqpnt").Where(seqpnt => seqpnt.Attribute("document") != null).Select(seqpnt => seqpnt.Attribute("document").Value).Distinct();
        }
        ...
        public IEnumerable<string> ClassesInAssembly(string assemblyname)
        {
            ...
            return this.Report.Descendants("module").Where(module => module.Attribute("assembly").Value.Equals(assemblyname)).Elements("method").Select(method => method.Attribute("class").Value).Where(value => !value.Contains("__")).Distinct().OrderBy(name => name);   
        }       
        ...
        public IEnumerable<string> FilesOfClass(string assemblyname, string classname)
        {
            ...
            return this.Report.Descendants("module").Where(module => module.Attribute("assembly").Value.Equals(assemblyname)).Elements("method").Where(method => method.Attribute("class").Value.Equals(classname)).Elements("seqpnt").Select(seqpnt => seqpnt.Attribute("document").Value).Distinct().ToArray();          
        }
